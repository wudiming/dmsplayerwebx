...
/** 读取当前请求会话中的 QM cookies */
export const getQQMusicCookies = (): Record<string, string> => {
  return getSessionCookies("qqmusic");
};

/** 更新 QM cookies 并写入会话存储 */
export const mergeQQMusicCookies = (cookies: Record<string, string>): void => {
  const current = getQQMusicCookies();
  const updated = { ...current, ...cookies };
  saveSessionCookies("qqmusic", updated);
  invalidateSession();
  coreLog.info(`[qm-cookie] 已保存 Cookie (${Object.keys(updated).length} 个字段)`);
};
...
export const clearQQMusicCookies = (): void => {
  ...
  invalidateSession();
  coreLog.info("[qm-cookie] 已从会话中清空 QM Session");
};
