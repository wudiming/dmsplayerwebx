...
import { neteaseLog } from "../../adapters/logger";
...
const loadSession = (): Record<string, string> => {
  const session = getSessionCookies("netease");
  syncDeviceState(session);
  return session;
};
...
export const ensureNeteaseAnonymousSession = async (): Promise<void> => {
  ...
  anonymousSessionPromise = (async () => {
    try {
      ...
      const body = (result?.body || {}) as { token?: unknown; cookie?: string };
      const patch = parseSetCookie(result?.cookie ?? []);
      let token = typeof body.token === "string" ? body.token : patch.MUSIC_A;
      if (!token && typeof body.cookie === "string") {
        const match = body.cookie.match(/(?:^|;\s*)MUSIC_A=([^;]+)/);
        if (match) token = match[1];
      }
      if (!token) token = getAnonymousToken();
      if (token) {
      ...
      neteaseLog.warn("游客会话初始化受阻，将使用默认标识继续:", err);
    }
  })().finally(() => {
...
export const callNetease = async (
  ...
  // 读缓存（带上用户标识，避免不同用户或游客混用同一缓存）
  const cacheable = !NON_CACHEABLE.has(name);
  const userToken = session.MUSIC_U ? session.MUSIC_U.slice(-10) : (session.MUSIC_A ? session.MUSIC_A.slice(-10) : "");
  const cacheKey = cacheable ? `${buildCacheKey(name, params)}|${userToken}` : "";
  if (cacheable) {
  ...
  const paramsCookie =
    typeof params.cookie === "string"
      ...
      : (params.cookie as Record<string, string> | undefined);
  const query: Query = {
    ...params,
    cookie: { ...session, ...(paramsCookie || {}) },
  };
