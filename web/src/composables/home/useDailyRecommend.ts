...
export const useDailyRecommend = () => {
  ...
  const tryBuild = async (kind: HeroKind): Promise<HeroSource | null> => {
    try {
      if (kind === "daily") {
        const tracks = await data.ensureDailyRecommend();
        return toSource("daily", tracks);
      }
  ...
  /** 播放队列预览中的某一首（从该首开始播放整组） */
  const playPreviewTrack = async (previewIndex: number): Promise<void> => {
    const current = slide.value;
    if (!current || current.tracks.length === 0) return;
    const actualIndex = (current.featuredIndex + previewIndex) % current.tracks.length;
    await player.playFrom(current.tracks, actualIndex);
  };

  return { hero, loading, previewTracks, playAll, addToQueue, playPreviewTrack, load };
};
