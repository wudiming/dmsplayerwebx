...
export const fetchDailySongs = async (): Promise<Track[]> => {
  ...
  const rawSongs = (body?.data?.dailySongs ?? (body as any)?.recommend ?? (body as any)?.data ?? []) as any[];
  const reasons = ((body?.data?.recommendReasons ?? (body as any)?.recommendReasons ?? []) as Array<{
  ...
  for (const r of reasons) {
    const sId = r.songId ?? r.id;
    if (sId != null && r.reason) reasonMap.set(String(sId), r.reason);
  }
  const enriched = rawSongs.map((song) => {
    const reason = reasonMap.get(String(song?.id));
    if (reason && !song.reason) {
      return { ...song, reason };
    }
    return song;
  });
  return songsToTracks(enriched);
};
...
/** 首页推荐区块展示数（支持宽屏整整两行充满） */
const HOME_GRID_LIMIT = 24;
/** personalized 拉取数 */
const PERSONALIZED_FETCH_LIMIT = 30;
