...
/** 新建成功后跳转到该歌单，并自动展开我的歌单 */
const handleCreated = (playlistId: string): void => {
  status.myPlaylistSource = "online";
  myPlaylistsCollapsed.value = false;
  router.push(`/collection/netease/playlist/${playlistId}`);
};
...
const renderMyHeader = () =>
  h(
    ...
    {
      class:
        "flex items-center justify-between gap-1 pl-3 pr-1 min-h-10 select-none group/my-header",
    },
    [
      h(
        "button",
        {
          type: "button",
          class:
            "flex items-center gap-1.5 cursor-pointer text-on-surface-variant/70 hover:text-on-surface transition-colors py-1 bg-transparent border-0 text-left outline-none",
          title: myPlaylistsCollapsed.value ? t("common.expand") : t("common.collapse"),
          onClick: () => {
        ...
        [
          h(
            "span",
            { class: "text-sm text-inherit leading-none select-none font-medium" },
            t("collection.my", { type: t("collection.playlist") }),
          ),
          h(IconLucideChevronDown, {
            class: [
              "size-3.5 transition-transform duration-200 text-on-surface-variant/60 group-hover/my-header:text-on-surface",
              myPlaylistsCollapsed.value ? "-rotate-90" : "rotate-0",
            ],
          }),
        ],
      ),
      h(
        SButton,
        {
          ...
          size: 26,
          round: true,
          title: t("playlist.create.title", "新建歌单"),
...
/** 「收藏的歌单」分组头部（带收起/展开折叠切换） */
const renderSubscribedHeader = () =>
  h(
    "div",
    {
      class:
        "flex items-center justify-between pl-3 pr-2 min-h-10 select-none group/sub-header",
    },
    [
      h(
        "button",
        {
          ...
          class:
            "flex items-center gap-1.5 cursor-pointer text-on-surface-variant/70 hover:text-on-surface transition-colors py-1 bg-transparent border-0 text-left outline-none",
          title: subscribedPlaylistsCollapsed.value ? t("common.expand") : t("common.collapse"),
          onClick: () => {
            subscribedPlaylistsCollapsed.value = !subscribedPlaylistsCollapsed.value;
          },
        },
        [
          h(
            "span",
            { class: "text-sm text-inherit leading-none font-medium" },
            t("collection.subscribed", { type: t("collection.playlist") }),
          ),
          h(IconLucideChevronDown, {
            class: [
              "size-3.5 transition-transform duration-200 text-on-surface-variant/60 group-hover/sub-header:text-on-surface",
              subscribedPlaylistsCollapsed.value ? "-rotate-90" : "rotate-0",
            ],
          }),
        ],
      ),
...
const myPlaylistItems = computed<SMenuItem[]>(() => {
  ...
  const items: SMenuItem[] = userStore.createdPlaylists.slice(1).map((pl) => ({
  ...
  return applySavedOrder(items, appearance.sidebarPlaylistOrder.myOnline).filter(
...
const menuItems = computed<SMenuItem[]>(() => {
  ...
  if (showSubscribed) {
    ...
    if (!subscribedPlaylistsCollapsed.value) {
...
const activeKey = computed(() => {
  ...
  // 发现音乐分部
  if (route.path.startsWith("/discover/playlists")) return "/discover/playlists";
  if (route.path.startsWith("/discover/artists") || route.path.startsWith("/artist/netease/"))
    return "/discover/artists";
  if (route.path.startsWith("/discover/toplists")) return "/discover/toplists";
  if (route.path.startsWith("/discover/new") || route.path.startsWith("/collection/netease/album/"))
    return "/discover/new";
  // 专辑详情页归属专辑列表
  ...
  // 歌手详情页归属歌手列表
  if (route.path.startsWith("/artist/local/")) return "/discover/artists";
