 ...
 * @param context - 本次播放的来源上下文
 */
export const loadTrack = async (track: Track | null, context?: PlaybackContext): Promise<void> => {
  if (!track) return;
  // Fuck DJ Mode
 ...
 * @param isPlayAll - 是否为“播放全部”触发（在随机播放模式下，播放全部会随机挑选一首作为首曲）
 */
export const playFrom = async (
  ...
  if (status.shuffleMode === "on") {
    // 随机模式下：
    // 若为“播放全部”且列表有多首，随机挑选一首作为首曲
    // 若用户明确点击了列表中的某首（包括点击第 0 首），严格以选中的歌曲为首曲
    const targetIdx =
      isPlayAll && items.length > 1
        ? Math.floor(Math.random() * items.length)
        : idx;
    status.playIndex = targetIdx;
  } else {
    status.playIndex = idx;
  }

  const current = status.currentTrack;
  const isSameTrack = media.track?.id === current?.id;
  if (isSameTrack) {
    if (!status.isPlaying) play();
  } else if (current) {
    await loadTrack(current, status.currentPlaybackContext);
  }
};

/**
 * 播放全部曲目
 * 随机模式下随机挑选首曲并打乱，顺序模式下从第 0 首起播
 * @param items - 歌曲列表
...
export const playAll = async (
  ...
  await playFrom(items, 0, context, true);
};
...
export const nextTrack = async (): Promise<void> => {
  ...
  const total = queue.queueLength.value;
  if (total === 0) return;
  ...
  if (status.shuffleMode === "on") {
    ...
    if (total <= 1) {
      ...
      // 记录当前播放索引到历史栈
      if (status.playIndex >= 0 && status.playIndex < total) {
        shuffleHistory.push(status.playIndex);
        if (shuffleHistory.length > 100) shuffleHistory.shift();
      }
      // 在剩余歌曲中随机挑选一首（避免与当前歌曲重复）
      let nextIndex = Math.floor(Math.random() * (total - 1));
      if (nextIndex >= status.playIndex) {
        nextIndex++;
      }
      status.playIndex = nextIndex;
    }
  } else {
    // 顺序播放模式：严格与歌单顺序同步
    status.playIndex = (status.playIndex + 1) % total;
  }
  await loadTrack(status.currentTrack, status.currentPlaybackContext);
};
...
export const playAtIndex = async (index: number): Promise<void> => {
  ...
  // 退出 FM
  status.fmMode = false;
  if (status.shuffleMode === "on" && status.playIndex >= 0 && status.playIndex < queue.queueLength.value) {
    shuffleHistory.push(status.playIndex);
    if (shuffleHistory.length > 100) shuffleHistory.shift();
  }
...
export const prevTrack = async (): Promise<void> => {
  ...
  if (status.shuffleMode === "on") {
    // 随机播放模式：优先从历史记录栈返回上一首刚播过的歌
    let targetIndex: number | null = null;
    while (shuffleHistory.length > 0) {
      const candidate = shuffleHistory.pop()!;
      if (candidate >= 0 && candidate < total && candidate !== status.playIndex) {
        targetIndex = candidate;
        break;
      }
    }
    if (targetIndex !== null) {
      status.playIndex = targetIndex;
    } else if (total > 1) {
      // 历史为空时，随机挑一首非当前曲
      let randIndex = Math.floor(Math.random() * (total - 1));
      if (randIndex >= status.playIndex) {
        randIndex++;
      }
      status.playIndex = randIndex;
    } else {
      status.playIndex = 0;
    }
  } else {
    // 顺序播放模式：严格与歌单顺序同步
    status.playIndex = (status.playIndex - 1 + total) % total;
  }
  await loadTrack(status.currentTrack, status.currentPlaybackContext);
};
...
export const setRepeatMode = (mode: RepeatMode): void => {
  ...
  // 若用户切换到单曲循环，且当前开启了随机播放，则关闭随机播放，避免单曲循环打断随机流程
  if (mode === "one" && status.shuffleMode === "on") {
    status.shuffleMode = "off";
    shuffleHistory.length = 0;
  }
  syncPlayMode();
...
export const toggleShuffleMode = (): void => {
  const status = useStatusStore();
  // 单曲循环模式下禁止切换随机播放
  if (status.repeatMode === "one") return;
...
export const setShuffleMode = (mode: ShuffleMode): void => {
  const status = useStatusStore();
  // 心动模式或单曲循环下忽略
  if (status.heartMode || status.repeatMode === "one") return;
...
export const removeFromQueue = async (index: number): Promise<void> => {
  ...
  for (let i = shuffleHistory.length - 1; i >= 0; i--) {
    if (shuffleHistory[i] === index) {
      shuffleHistory.splice(i, 1);
    } else if (shuffleHistory[i] > index) {
      shuffleHistory[i]--;
    }
  }
  if (index < status.playIndex) {
...
export const initPlayer = async (): Promise<void> => {
  ...
  // 若持久化状态中同时存在单曲循环和随机播放，解除冲突优先切回列表循环
  if (status.repeatMode === "one" && status.shuffleMode === "on") {
    status.repeatMode = "list";
  }
  // 若之前有未还原的原始队列备份，执行恢复以确保歌单顺序正确
  if (queue.originalQueue.value) {
    const currentTrackId = status.currentTrack?.id ?? "";
    status.playIndex = queue.unshuffleQueue(currentTrackId);
  }
  // 恢复上次的音量和播放模式到主进程
