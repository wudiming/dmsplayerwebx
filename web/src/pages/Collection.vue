...
const type = computed<CollectionType>(() => props.type || (route.params.type as CollectionType));
...
const loadCollection = async (): Promise<void> => {
  ...
  try {
    await loadCollectionService(source.value, type.value, id.value, {
...
watch(
  () => [source.value, type.value, id.value],
  () => {
    void loadCollection();
  },
);
...
const handlePlayAll = () => {
  ...
  player.playAll(collection.value.tracks, playbackContext.value);
};
...
const moreMenuItems = computed<DropdownMenuItem[]>(() => {
  const isOnline = source.value !== "local" && source.value !== "streaming";
  const isLocal = source.value === "local";
  const list: DropdownMenuItem[] = [
    ...
    },
    {
      ...
      children: [
        {
          key: "copyTitle",
          label: t(`collection.context.${type.value}.copyTitle`),
          icon: markRaw(IconCopy),
        },
        {
          key: "copyId",
          label: t(`collection.context.${type.value}.copyId`),
          icon: markRaw(IconCopy),
          show: !isLocal,
        },
        {
          key: "copyUrl",
          label: t(`collection.context.${type.value}.copyUrl`),
          icon: markRaw(IconCopy),
          show: isOnline && type.value !== "cloud",
        },
      ],
    },
  ];
