...
watch(
  () => [user.isLoggedIn, user.likedPlaylistId] as const,
  ([loggedIn, plId]) => {
    if (!loggedIn || !plId) return;
    user.ensureLikedPlaylist();
  },
  { immediate: true },
);

const currentTracks = computed<Track[]>(() => user.likedPlaylistTracks);
...
const handlePlayAll = (): void => {
  ...
  player.playAll(currentTracks.value, playbackContext.value);
};
...
<template>
  <div class="flex flex-col h-full">
    ...
    <Transition name="fade" mode="out-in" :duration="150">
      ...
      <div
        v-else-if="user.likedPlaylistTracks.length > 0"
        key="online-list"
      ...
      <div
        v-else-if="user.likedPlaylistLoading"
        key="online-loading"
        class="flex-1 flex items-center justify-center"
      >
