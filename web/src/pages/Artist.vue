...
const source = computed<TrackSource>(() => props.source || (route.params.source as TrackSource));
...
const fallbackName = computed(() => props.name || (typeof route.query.name === "string" ? route.query.name : undefined));
...
const loadArtist = async (): Promise<void> => {
  ...
  try {
    await loadArtistService(source.value, id.value, {
      ...
      onUpdate: (next) => {
        ...
        if (
          next &&
          ((source.value === "netease" && next.tracks.length >= 50) ||
            (source.value === "qqmusic" && next.tracks.length < next.trackCount))
        ) {
...
const onReachBottom = async (): Promise<void> => {
  if (
    (source.value !== "netease" && source.value !== "qqmusic") ||
    !hasMoreSongs.value ||
    loadingMore.value ||
    !artist.value
  )
  ...
  try {
    const { tracks, more } =
      source.value === "qqmusic"
        ? await fetchQQMusicArtistSongs(decodeURIComponent(id.value), current.tracks.length)
        : await fetchArtistSongs(decodeURIComponent(id.value), current.tracks.length);
    ...
    artist.value = {
      ...
      trackCount: source.value === "qqmusic" ? current.trackCount : current.tracks.length + tracks.length,
    };
...
const playbackContext = computed<PlaybackContext>(() => ({
  ...
  originId: decodeURIComponent(id.value),
  originType: "artist",
...
const handlePlayAll = () => {
  ...
  player.playAll(artist.value.tracks, playbackContext.value);
};
...
const modalActiveTab = ref<ArtistTab>("songs");

/** 当前 tab */
const activeTab = computed<ArtistTab>(() => {
  if (props.isModal) return modalActiveTab.value;
...
const onTabSwitch = (key: string): void => {
  if (props.isModal) {
    modalActiveTab.value = key as ArtistTab;
  } else {
        ...
        <Transition :name="tabTransitionName" mode="out-in">
          ...
          <div v-else-if="activeTab === 'albums'" key="albums" class="flex-1 min-h-0">
            <CoverList
              ...
              :padding-x="20"
              :padding-bottom="24"
              @click="(item) => navigateToAlbum(item.title, { source: source.value, albumId: item.id })"
            />
          </div>
        </Transition>
      </div>
