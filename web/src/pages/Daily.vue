...
const playbackContext = computed<PlaybackContext | undefined>(() => ({
  provider: "netease",
  originId: selectedDay.value ? `daily:${selectedDay.value.key}` : "daily:today",
  originType: "playlist",
  originName:
...
const handlePlayAll = (): void => {
  ...
  if (tracks.length > 0) player.playAll(tracks, playbackContext.value);
};
...
const moreMenuItems = computed<DropdownMenuItem[]>(() => [
  ...
  { key: "addQueue", label: t("home.hero.addQueue") || "添加到队列", icon: markRaw(IconLucideListPlus) },
  { key: "batch", label: t("songList.batch.manage"), icon: markRaw(IconLucideListChecks) },
]);

/** 处理更多操作 */
const handleMore = async (key: string): Promise<void> => {
  if (key === "refresh") {
  ...
  } else if (key === "addQueue") {
    const tracks = selectedDay.value?.tracks ?? [];
    if (tracks.length > 0) {
      const added = player.insertManyToQueue(tracks);
        ...
        <div class="flex min-w-0 flex-1 flex-col gap-2">
          ...
          <div class="mt-1 flex items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2.5">
              ...
              >
                <template #prefix>
                  <IconLucideSearch class="size-3.5 text-on-surface-variant/40 shrink-0" />
                </template>
              </SInput>
