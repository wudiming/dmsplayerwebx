...
/**
 * 洗牌：备份当前顺序到 originalQueue（仅首次），将 keepIndex 处的歌曲置于首位（若传 -1 则全打乱），其余随机打乱
 * @param keepIndex - 保持在首位的歌曲索引（通常是当前正在播放的歌，传 -1 时不保留首曲全量打乱）
 */
export const shuffleQueue = (keepIndex = -1): void => {
  ...
  if (keepIndex >= 0 && keepIndex < currentQueue.length) {
    const keepTrack = currentQueue[keepIndex];
    const rest = currentQueue.filter((_, index) => index !== keepIndex);
    shuffleArray(rest);
    queueEntries.value = [keepTrack, ...rest];
  } else {
    const all = [...currentQueue];
    shuffleArray(all);
    queueEntries.value = all;
  }
  save();
};
