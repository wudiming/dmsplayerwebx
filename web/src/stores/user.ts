...
import type { UserProfile, UserRadio, UserSubcount, UserVideo } from "@/types/user";
...
export const useUserStore = defineStore(
  ...
  () => {
    ...
    /** 收藏视频/MV */
    const videos = shallowRef<UserVideo[]>([]);
    /** 收藏播客/电台 */
    const radios = shallowRef<UserRadio[]>([]);
    /** 用户等级 */
    ...
    const invalidateSession = async (): Promise<void> => {
      ...
      resetAccountState();
      try {
        localStorage.removeItem("splayer_mock_user");
      } catch {}
    ...
    const ensureLikedPlaylist = async (force = false): Promise<void> => {
      ...
      if (!playlistId && profile.value?.userId) {
        await fetchAndApplyPlaylists(profile.value.userId);
      ...
      if (currentLikedPlaylistId !== playlistId) {
        ...
        if (likedPlaylistTracks.value.length === 0 || force || !isLikedPlaylistFresh()) {
          await refreshLikedPlaylist(playlistId);
        } else {
          void refreshLikedPlaylist(playlistId);
        }
        return;
      }
      if (force || !isLikedPlaylistFresh() || likedPlaylistTracks.value.length === 0) {
        await refreshLikedPlaylist(playlistId);
      }
    };
    ...
    const loadContent = async (uid: number): Promise<void> => {
      ...
      const [_plRes, likeRes, albumRes, artistRes, levelRes, videoRes, radioRes] = settled;
      ...
      if (videoRes.status === "fulfilled") videos.value = videoRes.value;
      if (radioRes.status === "fulfilled") radios.value = radioRes.value;
    ...
    /** 收藏 / 取消收藏视频 */
    const toggleVideoSubscribe = async (videoId: string, subscribe: boolean): Promise<void> => {
      await subscribeVideo(videoId, subscribe);
      videos.value = await fetchUserVideos();
    };

    /** 收藏 / 取消收藏播客 */
    const toggleRadioSubscribe = async (radioId: string, subscribe: boolean): Promise<void> => {
      await subscribeRadio(radioId, subscribe);
      radios.value = await fetchUserRadios();
    };
